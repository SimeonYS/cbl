...
class CblPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cbl`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cbl WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cbl (date, title, link, content)"
